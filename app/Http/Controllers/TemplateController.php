...
class TemplateController extends Controller
{
    ...
    public function showAddProducts($id)
    {
        ...
        $products = $productsCollection->map(function($product) {
            return [
                ...
                'colors' => $product->colors->map->only(['name', 'value']),
                'product_images' => $product->productImages->map->only(['image_url']),
                'styling_tips' => $product->stylingTips->map->only(['title', 'description']),
                'model_info' => $product->modelInfo->map->only(['key', 'value']),
                'garment_details' => $product->garmentDetails->map->only(['key', 'value']),
                'size_chart' => $product->sizeChart->map->only(['size', 'measurements']),
                'fabric_details' => $product->fabricDetails->map->only(['key', 'value']),
                'care_instructions' => $product->careInstructions->map->only(['instruction']),
                'faqs' => $product->faqs->map->only(['question', 'answer']),
            ];
        });
    ...
    public function storeProduct(Request $request, $siteId)
    {
        $request->validate([
            ...
            'image_url' => 'nullable|image|mimes:webp|max:10240',
            'images.*' => 'nullable|image|mimes:webp|max:10240',
