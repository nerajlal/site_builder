        ...
        @if(count($orders) > 0)
            <div class="space-y-8">
                @foreach($orders as $order)
                    ...
                    @endphp
                    <div class="bg-white rounded-lg shadow-lg p-4 sm:p-6 text-gray-800">
                        ...
                        <div class="divide-y divide-gray-200">
                            @foreach($order->products as $orderProduct)
                                <div class="flex flex-col sm:flex-row items-start sm:items-center py-4">
                                    <a href="{{ route('template2.single-product2.customer', ['headerFooterId' => $headerFooter->id, 'productId' => $orderProduct->product->id]) }}" class="w-24 h-24 sm:w-20 sm:h-20 flex-shrink-0 overflow-hidden rounded-md border border-gray-200 mb-4 sm:mb-0">
