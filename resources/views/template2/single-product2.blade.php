        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div class="space-y-4">
                ...
                <div class="relative group">
                    <div class="aspect-[4/5] bg-gray-800 rounded-lg overflow-hidden">
                ...
                <div class="flex space-x-2 overflow-x-auto pb-2">
                    @if($product->images && is_array($product->images))
                        @foreach($product->images as $index => $image)
                            <button class="w-20 h-20 bg-gray-800 rounded-lg flex-shrink-0 border-2 {{ $loop->first ? 'border-pink-600' : 'border-transparent' }} hover:border-pink-600 thumbnail-button" onclick="changeImage('{{ $image }}', this)">
                                <img src="{{ $image }}" alt="Thumbnail {{ $index + 1 }}" class="w-full h-full object-cover rounded-md">
                            </button>
                        @endforeach
                    @endif
                    @if($product->video_url)
                        <button class="w-20 h-20 bg-gray-800 rounded-lg flex-shrink-0 border-2 border-transparent hover:border-pink-600 thumbnail-button" onclick="playVideo('{{ $product->video_url }}', this)">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    <div class="flex items-center space-x-4">
                        ...
                        @if($product->original_price && $product->original_price > $product->price)
                            <span class="text-xl text-gray-400 line-through">₹{{ number_format($product->original_price, 0) }}</span>
                            <span class="text-lg text-green-400 font-semibold bg-green-900/50 px-2 py-1 rounded">{{ round((($product->original_price - $product->price) / $product->original_price) * 100) }}% OFF</span>
                        @endif
                    </div>
                ...
                <div>
                    ...
                    <div class="flex space-x-3">
                        ...
                        <button class="color-btn w-10 h-10 rounded-full border-2 border-gray-700 shadow-md relative" style="background-color: {{ $color['hex'] }};" onclick="selectColor('{{ $color['name'] }}', this)" data-color-name="{{ $color['name'] }}">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-5 gap-2 my-2">
                        @foreach($product->sizes as $size)
                            <button class="size-btn py-3 text-center border border-gray-700 rounded text-gray-300 hover:border-pink-600" onclick="selectSize('{{ $size['size'] }}', {{ $size['stock'] }}, this)" data-size="{{ $size['size'] }}" data-stock="{{ $size['stock'] }}">
                                <span>{{ $size['size'] }}</span>
                                @if($size['stock'] == 0)
        ...
        <div class="mt-16">
            <div class="border-b border-gray-700">
                <nav class="-mb-px flex space-x-8 overflow-x-auto">
                    ...
                    @if(isset($product->details['care_instructions']))<button class="tab-btn py-4 px-1 border-b-2 border-transparent text-gray-400 hover:text-gray-200 whitespace-nowrap" onclick="showTab('care', this)">Care</button>@endif
                </nav>
            </div>
            <div class="mt-8">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(firstColor) {
            firstColor.classList.add('border-pink-600');
            firstColor.classList.remove('border-gray-700');
        }
    });
