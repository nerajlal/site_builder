@include('template2.head2', ['is_default' => $is_default, 'headerFooter' => $headerFooter])
    ...
    <div class="max-w-7xl mx-auto px-4 pb-12 text-gray-300">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div class="space-y-6">
                ...
                <div class="relative bg-purple-900/20 border border-purple-800 p-4 rounded-lg combo-offer-pulse overflow-hidden">
                    <canvas id="confetti-canvas-2" class="absolute top-0 left-0 w-full h-full z-10 pointer-events-none"></canvas>
        ...
        <div class="mt-16">
            <div class="border-b border-gray-700">
                <nav class="-mb-px flex space-x-8 overflow-x-auto">
                    ...
                    <button class="tab-btn py-4 px-1 border-b-2 border-transparent text-gray-400 hover:text-white whitespace-nowrap" onclick="showTab('reviews')">Reviews ({{ $product->reviews->count() }})</button>
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="grid md:grid-cols-3 gap-8">
                            <div class="bg-gray-800 p-6 rounded-lg text-center">
                                <div class="text-4xl font-bold mb-2 text-white">{{ number_format($averageRating, 1) }}</div>
                                ...
                                <p class="text-sm text-gray-400">Based on {{ $totalReviews }} reviews</p>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="space-y-2">
                                    @for ($i = 5; $i >= 1; $i--)
                                        <div class="flex items-center space-x-2">
                                            <span class="text-sm w-8 text-gray-300">{{ $i }}★</span>
                                            ...
                                            <span class="text-sm text-gray-400">{{ $totalReviews > 0 ? round(($ratingDistribution->get($i, 0) / $totalReviews) * 100) : 0 }}%</span>
                                        </div>
                                    @endfor
                                </div>
                            </div>
                            ...
                            @forelse($product->reviews as $review)
                                <div class="border-b border-gray-700 pb-6">
                                    <div class="flex items-start space-x-4">
                                        <div class="w-12 h-12 bg-primary text-white rounded-full flex items-center justify-center font-semibold">{{ strtoupper(substr($review->customer->name, 0, 1)) }}</div>
                                        <div class="flex-1">
                                            <div class="flex items-center justify-between mb-2">
                                                <div>
                                                    <h4 class="font-semibold text-white">{{ $review->customer->name }}</h4>
                                            ...
                                            <p class="text-gray-300 mb-3">{{ $review->review }}</p>
                                        </div>
                                    </div>
                                </div>
                            @empty
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const canvas = document.getElementById('confetti-canvas-2');
            if (canvas) {
