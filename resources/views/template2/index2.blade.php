    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      @else
        @foreach ($categories as $category)
          <a href="{{ route('template2.product2.customer', ['headerFooterId' => $headerFooter->id, 'category_id' => $category->id]) }}" class="group text-center p-8 transition-all duration-300 ease-in-out bg-gray-900 hover:bg-gray-800 rounded-lg border border-gray-800 hover:border-pink-600">
            <h4 class="text-xl font-medium text-white">{{ $category->name }}</h4>
            <p class="text-center mt-4 text-md font-light text-pink-500 group-hover:text-pink-400 transition-colors duration-300">Shop Now</p>
          </a>
        @endforeach
      @endif
    </div>
