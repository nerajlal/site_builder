    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-white">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between mb-8 bg-gray-900 p-4 rounded-lg border border-gray-700">
            <form action="{{ url()->current() }}" method="GET" class="grid grid-cols-2 md:flex md:flex-row items-center gap-4 md:space-x-4 w-full">
                ...
                <div class="relative w-full">
                    <select name="min_rating" onchange="this.form.submit()" class="w-full px-4 py-2 border border-gray-600 rounded-lg hover:border-yellow-500 focus:border-yellow-500 focus:ring-yellow-500 transition-colors bg-gray-800 text-white">
                ...
                <button type="submit" class="w-full md:w-auto px-4 py-2 bg-yellow-500 text-black rounded-lg hover:bg-yellow-600 col-span-2">Filter</button>
            </form>
        </div>
