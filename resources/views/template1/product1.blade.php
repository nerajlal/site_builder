        ...
        <div class="flex flex-col md:flex-row items-center justify-between mb-8 bg-gray-50 p-4 rounded-lg">
            <form action="{{ url()->current() }}" method="GET" class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
                <div class="relative w-full md:w-auto">
                    <select name="sort" onchange="this.form.submit()" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-100">
                ...
                <div class="relative w-full md:w-auto">
                    <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min Price" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg">
                </div>
                <div class="relative w-full md:w-auto">
                    <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Max Price" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg">
                </div>
                <button type="submit" class="w-full md:w-auto px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700">Filter</button>
            </form>
        </div>
...
@else
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <!-- Filters Bar -->
        <div class="flex flex-col md:flex-row items-center justify-between mb-8 bg-white p-4 rounded-lg shadow-sm">
            <form action="{{ url()->current() }}" method="GET" class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
                <div class="relative w-full md:w-auto">
                    <select name="sort" onchange="this.form.submit()" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                        <option value="">Sort by</option>
                        <option value="price_asc" {{ request('sort') == 'price_asc' ? 'selected' : '' }}>Price: Low to High</option>
                        <option value="price_desc" {{ request('sort') == 'price_desc' ? 'selected' : '' }}>Price: High to Low</option>
                    </select>
                ...
                <div class="relative w-full md:w-auto">
                    <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min Price" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg">
                </div>
                <div class="relative w-full md:w-auto">
                    <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Max Price" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg">
                </div>
                <button type="submit" class="w-full md:w-auto px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">Filter</button>
            </form>
        </div>
