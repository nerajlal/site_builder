...
<div class="bg-gray-100 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-lg p-6">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Your Wishlist ({{ count($wishlistItems) }} items)</h2>
                ...
                <div id="wishlist-items-container" class="divide-y divide-gray-200">
                    @foreach($wishlistItems as $item)
                        <div class="wishlist-item flex items-center py-6" data-id="{{ $item->id }}">
                            <a href="{{ route('template4.single-product4.customer', ['headerFooterId' => $headerFooter->id, 'productId' => $item->product->id]) }}" class="w-24 h-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                            ...
                            <div class="ml-4 flex flex-1 flex-col">
                                <div>
                                    <div class="flex justify-between text-base font-medium text-gray-900">
                                        <h3>
                                            <a href="{{ route('template4.single-product4.customer', ['headerFooterId' => $headerFooter->id, 'productId' => $item->product->id]) }}">{{ $item->product->name }}</a>
                                        </h3>
                                ...
                                <div class="flex flex-1 items-end justify-between text-sm mt-4">
                                    ...
                                    <div class="flex">
                                        <a href="{{ route('template4.single-product4.customer', ['headerFooterId' => $headerFooter->id, 'productId' => $item->product->id]) }}" class="add-to-cart-btn font-medium text-white bg-pink-600 hover:bg-pink-700 px-4 py-2 rounded-md">View Product</a>
                                    </div>
                                </div>
                            </div>
                        </div>
