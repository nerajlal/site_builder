@include('template4.head4')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div class="space-y-4">
                ...
                <div class="flex space-x-2 overflow-x-auto pb-2" id="thumbnail-gallery">
                    <button class="thumbnail-btn w-20 h-20 bg-pink-100 rounded-lg flex-shrink-0 border-2 border-[#9333ea]">
                        <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-full h-full object-cover rounded-lg">
                    </button>
                    @foreach($productImages as $image)
                    <button class="thumbnail-btn w-20 h-20 bg-pink-50 rounded-lg flex-shrink-0 border border-gray-200 hover:border-[#9333ea]">
                        <img src="{{ $image->image_url }}" alt="{{ $product->name }}" class="w-full h-full object-cover rounded-lg">
                    </button>
                    @endforeach
                    @if($product->video_url)
                    <a href="{{ $product->video_url }}" target="_blank" class="w-20 h-20 bg-pink-50 rounded-lg flex-shrink-0 border border-gray-200 hover:border-[#9333ea] flex items-center justify-center text-gray-400">
                        <i class="fas fa-video text-[#9333ea]"></i>
                    </a>
                    @endif
                </div>
                ...
                <div class="bg-white rounded-lg p-4 mt-6">
                    <h3 class="text-sm font-semibold mb-3 flex items-center">
                        <i class="fab fa-instagram text-[#9333ea] mr-2"></i>
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button class="text-sm text-[#7e22ce] hover:underline" onclick="openSizeGuide()">Size Guide</button>
                    </div>
                    <div class="grid grid-cols-6 gap-2 mb-2">
                        @foreach($sizes as $size)
                            @if($size->in_stock)
                                <button class="size-btn py-3 text-center border rounded hover:border-[#7e22ce]"
                ...
                <div class="relative bg-purple-50 border border-purple-200 p-4 rounded-lg combo-offer-pulse overflow-hidden">
                    <canvas id="confetti-canvas-4" class="absolute top-0 left-0 w-full h-full z-10 pointer-events-none"></canvas>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="flex items-center space-x-2 mb-3">
                        <input type="text" 
                               ...
                               class="flex-1 px-3 py-2 border rounded focus:outline-none focus:border-[#9333ea]"
                               id="pinCodeInput"
                               maxlength="6">
                        <button class="px-6 py-2 bg-[#7e22ce] text-white rounded hover:bg-[#9333ea] transition-colors" onclick="checkPinCode()">Check</button>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button class="py-4 border-2 border-[#9333ea] text-[#7e22ce] rounded-lg font-semibold hover:bg-[#7e22ce] hover:text-white transition-colors" onclick="addToCart()">
                            Add to Cart
                        </button>
                        <button class="py-4 bg-[#7e22ce] text-white rounded-lg font-semibold hover:bg-pink-700 transition-colors" onclick="buyNow()">
                            Buy Now
                        </button>
                    </div>
                    ...
                    <div class="flex justify-center space-x-8 pt-4">
                        <button class="flex items-center space-x-2 text-gray-600 hover:text-[#7e22ce] transition-colors" onclick="toggleWishlist(this)" data-product-id="{{ $product->id }}">
                            <i class="{{ $inWishlist ? 'fas' : 'far' }} fa-heart text-lg"></i>
                            <span>{{ $inWishlist ? 'Wishlisted' : 'Wishlist' }}</span>
                        </button>
                        <button class="flex items-center space-x-2 text-gray-600 hover:text-[#7e22ce] transition-colors" onclick="shareProduct()">
                            <i class="fas fa-share-alt text-lg"></i>
                            <span>Share</span>
                        </button>
                        <button class="flex items-center space-x-2 text-gray-600 hover:text-[#7e22ce] transition-colors" onclick="compareProduct()">
        ...
        <div id="stickyBar" class="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg p-4 hidden lg:hidden z-30">
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-2">
                    <button class="px-4 py-2 border border-[#9333ea] text-[#7e22ce] rounded" onclick="addToCart()">Add to Cart</button>
                    <button class="px-4 py-2 bg-[#7e22ce] text-white rounded" onclick="buyNow()">Buy Now</button>
                </div>
            </div>
        </div>
                            ...
                            @foreach($product->faqs as $faq)
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    <h4 class="font-semibold mb-2 flex items-start">
                                        <i class="fas fa-question-circle text-[#7e22ce] mr-3 mt-1"></i>
                    ...
                    <div class="mt-8 bg-gray-50 p-6 rounded-lg">
                        ...
                        <div class="grid md:grid-cols-4 gap-4 text-sm">
                            <div class="text-center">
                                <div class="w-12 h-12 bg-[#7e22ce] text-white rounded-full flex items-center justify-center mx-auto mb-2">1</div>
                                <p class="font-medium">Contact Us</p>
                                <p class="text-gray-600">Via WhatsApp or email within 7 days</p>
                            </div>
                            <div class="text-center">
                                <div class="w-12 h-12 bg-[#7e22ce] text-white rounded-full flex items-center justify-center mx-auto mb-2">2</div>
                                <p class="font-medium">Schedule Pickup</p>
                                <p class="text-gray-600">We'll arrange free pickup from your location</p>
                            </div>
                            <div class="text-center">
                                <div class="w-12 h-12 bg-[#7e22ce] text-white rounded-full flex items-center justify-center mx-auto mb-2">3</div>
                                <p class="font-medium">Quality Check</p>
                                <p class="text-gray-600">We verify the product condition</p>
                            </div>
                            <div class="text-center">
                                <div class="w-12 h-12 bg-[#7e22ce] text-white rounded-full flex items-center justify-center mx-auto mb-2">4</div>
    ...
    <script>
        ...
        function showTab(tabName) {
            ...
            tabButtons.forEach(button => {
                button.classList.remove('active', 'border-[#9333ea]', 'text-pink-600');
            ...
            event.target.classList.add('active', 'border-[#9333ea]', 'text-pink-600');
        ...
        function selectColor(color, colorName) {
            ...
            colorButtons.forEach(button => {
                ...
                if (buttonColor === color) {
                    button.classList.add('border-[#9333ea]');
                    button.classList.remove('border-gray-200');
                    if (checkIcon) checkIcon.style.display = 'flex';
                } else {
                    button.classList.remove('border-[#9333ea]');
        ...
        function selectSize(size) {
            ...
            sizeButtons.forEach(button => {
                ...
                if (buttonSize === size) {
                    button.classList.add('border-2', 'border-[#9333ea]', 'bg-[#7e22ce]', 'text-white');
                    button.classList.remove('border', 'border-gray-200');
                } else if (!button.disabled) {
                    button.classList.remove('border-2', 'border-[#9333ea]', 'bg-[#7e22ce]', 'text-white');
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const canvas = document.getElementById('confetti-canvas-4');
            if (canvas) {
