...
<div class="flex-1 overflow-y-auto">
    <main class="flex-1 overflow-y-auto p-6">
        <div class="mb-6 flex flex-col md:flex-row items-start md:items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Select Your Site</h1>
                <p class="text-gray-600">Select the website to add products</p>
            </div>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse ($headerFooters as $site)
                <a href="{{ url('/addproducts/' . $site->id) }}"
            ...
            @empty
                <div class="col-span-full text-center text-gray-500">
                    No sites available. Please create one.
                </div>
            @endforelse
        </div>
    </main>
